jQuery(document).ready(function ($) {
            ...
            if (properties[i].htmlAttr) {
                ...
                switch (properties[i].htmlAttr) {
                    ...
                    case 'childStyle':
                        ...
                        let childCss;
                        childCss = ViWec.StyleManager.getStyle(_element, properties[i], _key);

                        if (childCss) {
                            childStyle[properties[i].target][_key] = viWecFixColor(childCss);
                        }
                        break;
        ...
        if (properties) {
            for (let i in properties) {
                if (properties[i].htmlAttr) {
                    ...
                    if (css) {
                        // if (!['0px', 0].includes(css) && css.indexOf('0px none') === -1 && css.indexOf('0px solid') === -1) {
                        style[_key] = viWecFixColor(css);
                        // }
                    }
                }
    ...
    const getEmailStructure = () => {
        ...
        $(viWecEditorArea).find('.viwec-layout-row').each(function (rowIndex, row) { //loop rows
            ...
            // rowInnerStyle = viWecGetRowCss($(row).find('.viwec-layout-inner'), type);
