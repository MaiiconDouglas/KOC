...
class Email_Trigger {
	...
	protected $object;
	protected $ignore_style = false;
	protected $use_default_temp = false;
	protected $class_email;
	protected $heading;

	private function __construct() {
//		add_filter( 'woocommerce_email_classes', array( $this, 'email_classes' ), 9999 );
		add_filter( 'wc_get_template', array( $this, 'replace_template' ), 10, 5 );
		add_action( 'viwec_email_template', array( $this, 'load_template' ), 10 );
		add_action( 'woocommerce_email', array( $this, 'get_email_ids' ) );
		add_filter( 'woocommerce_email_styles', array( $this, 'replace_wc_style' ), 9999 );
		...
		add_action( 'woocommerce_email_header', array( $this, 'get_heading' ) );

	}

	public function get_heading( $a ) {
		$this->heading = $a;
	}
...
//		$this->remove_header_footer_hook( $this->use_default_temp );
	...
	public function replace_template( $located, $template_name, $args, $template_path, $default_path ) {
		...
		if ( ! empty( $args['email']->id ) && in_array( $args['email']->id, $unique ) ) {
			return $located;
		}
		...
		if ( isset( $args['email'] ) && ! empty( $args['email']->id ) ) {
			...
			if ( $this->template_id ) {
				$this->ignore_style = true;
				$unique[]           = $args['email']->id;
	...
	public function replace_wc_style( $css ) {
		if ( $this->template_id ) {
			$css = '';
		}
