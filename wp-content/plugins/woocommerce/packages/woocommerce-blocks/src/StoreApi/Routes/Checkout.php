...
class Checkout extends AbstractRoute {
		...
		return [
			...
			[
				...
				'args'                => array_merge(
					[
						'payment_data' => [
							'description' => __( 'Dados para passar para o meio de pagamento no processamento do pagamento.', 'woocommerce' ),
							'type'        => 'array',
							'items'       => [
								'type'       => 'object',
								'properties' => [
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		if ( ! $order_object ) {
			throw new RouteException(
				...
				__( 'Esta sessão não tem pedidos pendentes de pagamento.', 'woocommerce' ),
				500
			);
		}
		...
		switch ( $error->getMessage() ) {
			case 'registration-error-invalid-email':
				throw new RouteException(
					...
					__( 'Por favor, forneça um endereço de e-mail válido.', 'woocommerce' ),
					400
				);
			...
			case 'registration-error-email-exists':
				throw new RouteException(
					...
					apply_filters(
						...
						__( 'Uma conta já está registrada com seu endereço de e-mail. Por favor entre.', 'woocommerce' )
					),
					400
				);
		}
	...
	protected function process_without_payment( \WC_Order $order, \WP_REST_Request $request ) {
		...
		$result = new PaymentResult( 'Sucesso' ); //*sucesso* sucess modificado//
	...
	protected function process_payment( \WC_Order $order, \WP_REST_Request $request ) {
		...
		$order->update_status( 'Pendente' ); //Pendente
		...
		try {
			...
			if ( ! $result instanceof PaymentResult ) {
				throw new RouteException( 'woocommerce_rest_checkout_invalid_payment_result', __( 'Resultado de pagamento inválido recebido da forma de pagamento.', 'woocommerce' ), 500 );
			}
	...
	protected function get_request_payment_method_id( \WP_REST_Request $request ) {
		...
		if ( empty( $payment_method ) ) {
			throw new RouteException(
				...
				__( 'Nenhum método de pagamento fornecido.', 'woocommerce' ),
				400
			);
		}
		...
		if ( ! in_array( $payment_method, $valid_methods, true ) ) {
			throw new RouteException(
				...
				sprintf(
					...
					__( 'Fornecido método de pagamento inválido. Forneça um dos seguintes: %s', 'woocommerce' ),
	...
	protected function get_request_payment_method( \WP_REST_Request $request ) {
		...
		if ( ! $payment_method_object || ! wc_string_to_bool( $payment_method_object->enabled ) ) {
			throw new RouteException(
				...
				__( 'Este portal de pagamento não está disponível.', 'woocommerce' ),
				400
			);
		}
