...
class CartUpdateShipping extends AbstractCartRoute {
	...
	public function get_args() {
		return [
			[
				...
				'args'                => [
					'first_name' => [
						'description'       => __( 'Nome do cliente.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
					...
					'last_name'  => [
						'description'       => __( 'Sobrenome do cliente.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
					...
					'address_1'  => [
						'description'       => __( 'Primeira linha do endereço de envio.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
					...
					'address_2'  => [
						'description'       => __( 'Segunda linha do endereço de envio.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
					...
					'city'       => [
						'description'       => __( 'Cidade do endereço de envio.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
					...
					'state'      => [
						'description'       => __( 'Código ISO, ou nome, do estado, província ou distrito do endereço para o qual está sendo enviado.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
					...
					'postcode'   => [
						'description'       => __( 'CEP ou código postal do endereço de envio para.', 'woocommerce' ),
						'type'              => 'string',
						'default'           => '',
						'sanitize_callback' => 'wc_clean',
						'validate_callback' => 'rest_validate_request_arg',
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		if ( ! wc_shipping_enabled() ) {
			throw new RouteException( 'woocommerce_rest_shipping_disabled', __( 'O envio está desativado.', 'woocommerce' ), 404 );
		}
	...
	public function validate_shipping_address( $request ) {
		...
		if ( empty( $request['country'] ) ) {
			throw new RouteException(
				...
				sprintf(
					...
					__( 'No código do país de destino foi fornecido. Forneça um dos seguintes: %s', 'woocommerce' ),
		...
		if (
			...
			throw new RouteException(
				...
				sprintf(
					/* translators: 1: valid country codes */
					__( 'O código do país de destino não é válido. Insira um dos seguintes: %s', 'woocommerce' ),
					implode( ', ', array_keys( $valid_countries ) )
				),
				400
			);
				...
				if ( ! in_array( $request['state'], $valid_state_values, true ) ) {
					throw new RouteException(
						...
						sprintf(
							...
							__( 'O estado de destino não é válido. Insira um dos seguintes: %s', 'woocommerce' ),
