...
<div id="reviews" class="electro-advanced-reviews">
	<div class="advanced-review row">
		<div class="col-xs-12 col-md-6">
			...
			<div class="rating-histogram">
				...
				<div class="rating-bar">
					<div class="star-rating" title="<?php printf( __( 'Avaliado %s out of 5', 'electro' ), $rating ); ?>">
			...
			<div id="review_form_wrapper">
				<div id="review_form">
					<?php
						...
						$comment_form = array(
							'title_reply'          => have_comments() ? __( 'Add a review', 'electro' ) : sprintf( __( 'Seja o primeiro a avaliar &ldquo;%s&rdquo;', 'electro' ), get_the_title() ),
							'title_reply_to'       => __( 'Deixe uma resposta para %s', 'electro' ),
							...
							'label_submit'  => __( 'Adicione uma avaliação', 'electro' ),
							'logged_in_as'  => '',
							'comment_field' => ''
						);

						if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
							$comment_form['must_log_in'] = '<p class="must-log-in">' .  sprintf( __( 'Você deve ser <a href="%s">logado</a> para postar um comentário.', 'electro' ), esc_url( $account_page_url ) ) . '</p>';
						}

						if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
							$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . __( 'Your Rating', 'electro' ) .'</label><Selecione o nome="rating" id="rating">
		...
		<?php else : ?>
			...
			<p class="woocommerce-verification-required"><?php _e( 'Apenas clientes logados que compraram este produto podem deixar um comentário.', 'electro' ); ?></p>
	...
	<div id="comments">
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php _e( 'Não há comentários ainda.', 'electro' ); ?></p>
